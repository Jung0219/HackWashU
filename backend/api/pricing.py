...
barnes_csv = "431452426_BarnesJewishStPetersHospital_standardcharges.csv"
if not os.path.exists(barnes_csv):
    barnes_csv = "/Users/jeongseyun7/Documents/Slupendous/HackWashU/431452426_BarnesJewishStPetersHospital_standardcharges.csv"

lincoln_csv = "Lincoln_20006_2025.csv"
if not os.path.exists(lincoln_csv):
    lincoln_csv = "/Users/jeongseyun7/Documents/Slupendous/HackWashU/Lincoln_20006_2025.csv"
...
try:
    ...
    print(f"✅ Loaded {len(PROCEDURES_DB['barnes_jewish'])} procedures from Barnes Jewish\n")
...
try:
    ...
    print(f"✅ Loaded {len(PROCEDURES_DB['lincoln'])} procedures from Lincoln\n")
except Exception as e:
    ...
    for proc in WOUND_PROCEDURE_MAPPING[wound_type]:
        ...
        if code in hospital_db:
            ...
            if min_p == 0 and max_p == 0:
                gross = float(row.get('standard_charge|gross', '').replace('"', '') or 0)
                discounted = float(row.get('standard_charge|discounted_cash', '').replace('"', '') or 0)
...
def compare_pricing():
    ...
    savings_percentage = (abs(savings_amount) / barnes_total * 100) if barnes_total > 0 else 0
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"\n🏥 Hospitals loaded:")
    print(f"  • Barnes Jewish St. Peters ({len(PROCEDURES_DB.get('barnes_jewish', {}))} procedures)")
    print(f"  • Mercy Hospital Lincoln ({len(PROCEDURES_DB.get('lincoln', {}))} procedures)")
